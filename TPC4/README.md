...
A ontologia foi populada a partir de dois datasets JSON (`dataset_temporal_100.json` e `dataset_temporal_v2_100.json`), totalizando 200 entradas, através do script `populate.py` com recurso à biblioteca `rdflib`. O script carrega a ontologia base (`biblioteca_temporal.ttl`), insere os indivíduos no grafo mapeando cada tipo à respetiva classe OWL, e serializa o resultado em `biblioteca_temporal_populated.ttl`.

Foram respondidas 10 questões obrigatórias e 3 questões bónus, explorando diferentes aspetos da ontologia, como a distribuição dos livros pelas linhas temporais, a identificação de autores e as suas obras e a deteção de situações anómalas.
...
| `biblioteca_temporal_populated.ttl` | Ontologia populada gerada pelo script |
